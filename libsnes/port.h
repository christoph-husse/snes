...
#define MSB_FIRST
...
#else
		throw "This method is not intended for big-endian.";
#endif
	...
	#define read_aligned(ptr, type) __read_aligned_internal<type>((ptr))
	#define write_aligned(ptr, type, val) __write_aligned_internal<type>((ptr), (val))

#define READ_WORD(s)		read_aligned(s, uint16)
#define READ_3WORD(s)		(read_aligned(s, uint32) & 0x00ffffff)
#define READ_DWORD(s)		read_aligned(s, uint32)
#define WRITE_WORD(s, d)	write_aligned(s, uint16, (d))
#define WRITE_3WORD(s, d)	write_aligned(s, uint16, (uint16) (d)), *((uint8 *) (s) + 2) = (uint8) ((d) >> 16)
#define WRITE_DWORD(s, d)	write_aligned(s, uint32, (d))
